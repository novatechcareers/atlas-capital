...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ error: error?.message || 'Unable to load deposits.' }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    return NextResponse.json({ error: error?.message || 'Unable to create deposit.' }, { status: 500 });
  }
}
